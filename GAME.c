...
int main(){
    ...
    SDL_Rect btnPular    = {xBtn, 170, larguraBtn, alturaBtn};
    SDL_Rect btnSairJogo = {xBtn, 240, larguraBtn, alturaBtn};
    ...
    const char* nomes[]={
    "Brasil","EUA","China","Russia","Alemanha","Japao","India","Egito","Franca","Italia",
    "Canada","Mexico","Australia","Argentina","Chile","Peru","Angola","Grecia","Turquia","Ira",
    "Iraque","Coreia","Espanha","Suecia","Congo",
    "Noruega","Finlandia","Polonia","Ucrania","Portugal","Holanda","Belgica","Suica","Austria","Hungria",
    "Dinamarca","Irlanda","Escocia","NovaZelandia","Indonesia","Filipinas","Vietna","Tailandia","Marrocos","Argelia",
    "Nigeria","Quenia","Etiopia","Colombia","Venezuela"
    };
                ...
                if(menu){
                    if(mouseSobre(btnJogar,mx,my)){
                        ...
                        for(int i=0;i<TOTAL_PAISES;i++){
                            ...
                            mapa[i].tropas=3;
                            mapa[i].retangulo=(SDL_Rect){20+(i%5)*120,20+(i/5)*40,120,40};
                        }
                    ...
                    if(mouseSobre(btnPular,mx,my)){modoAtaque=0; turno++; selecionado=NULL; tropasCalculadas=0;}
                    ...
                    for(int i=0;i<TOTAL_PAISES;i++){
                        ...
                        if(mouseSobre(p->retangulo,mx,my)){
                            if(modoAtaque){
                                ...
                                if(!selecionado && p->dono==id && p->tropas>1)
                                    selecionado=p;
                                    ...
                                    nDef = (p->tropas >= 3) ? 3 : p->tropas;

                                    rolarDados(dadosAtk,nAtk);
                                    rolarDados(dadosDef,nDef);

                                    // Ordenar Atacante (Maior para Menor)
                                    for(int a=0;a<nAtk-1;a++)
                                        for(int b=a+1;b<nAtk;b++)
                                            if(dadosAtk[a]<dadosAtk[b]){
                                                int t=dadosAtk[a]; dadosAtk[a]=dadosAtk[b]; dadosAtk[b]=t;
                                            }

                                    // Ordenar Defensor (Maior para Menor)
                                    for(int a=0;a<nDef-1;a++)
                                        for(int b=a+1;b<nDef;b++)
                                            if(dadosDef[a]<dadosDef[b]){
                                                int t=dadosDef[a]; dadosDef[a]=dadosDef[b]; dadosDef[b]=t;
                                            }

                                    // --- LÓGICA DE COMBATE CORRIGIDA ---
                                    int comparacoes = (nAtk < nDef) ? nAtk : nDef;

                                    // 1. Compara os dados que possuem par
                                    for(int j=0; j<comparacoes; j++){
                                        if(dadosAtk[j] > dadosDef[j])
                                            p->tropas--;
                                        else
                                            selecionado->tropas--;
                                    }

                                    // 2. Dados excedentes (Caso o Atacante tenha mais dados que o Defensor)
                                    if(nAtk > nDef && p->tropas > 0){
                                        int extras = nAtk - nDef;
                                        p->tropas -= extras;
                                    }

                                    // 3. Verifica conquista de território
                                    if(p->tropas <= 0){
                                        p->dono = id;
                                        p->tropas = 1;         // Ocupa com 1 tropa
                                        selecionado->tropas--; // Tira 1 da origem para mover
                                    }
        ...
        if(menu){
            ...
            for(int i=0;i<TOTAL_PAISES;i++){
                ...
                SDL_SetRenderDrawColor(r,
                    jogadores[p->dono].cor.r,
                    jogadores[p->dono].cor.g,
                    jogadores[p->dono].cor.b,255);
            ...
            char hud[80];
            sprintf(hud,"Vez: %s | Tropas: %d | %s",
                jogadores[id].nome,
                tropasDisponiveis,
                modoAtaque?"ATAQUE":"REFORCO");
            ...
            if(nAtk>0)
                mostrarDados(r,font,dadosAtk,nAtk,dadosDef,nDef);
        }
